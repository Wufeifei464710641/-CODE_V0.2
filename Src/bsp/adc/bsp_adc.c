...
void MX_ADCx_Init(void)
{
  ...
  hadcx1.Init.ContinuousConvMode = DISABLE;//注意不要使能连续转化，否则后面就回一直转换，直至count清0，
  hadcx1.Init.DiscontinuousConvMode = DISABLE;
  hadcx1.Init.ExternalTrigConv = ADC_EXTERNALTRIGCONV_T1_CC1; //外部触发启动 tim1_cc1作为触发源
  ...
  hadcx2.Init.ContinuousConvMode = DISABLE;//注意不要使能连续转化，否则后面就回一直转换，直至count清0，
  hadcx2.Init.DiscontinuousConvMode = DISABLE;
  hadcx2.Init.ExternalTrigConv = ADC_EXTERNALTRIGCONV_T1_CC1;  //外部触发启动 tim1_cc1作为触发源
