...
//		HAL_Delay(1000);	//0.2s		
    ...
    ADC_ConvertedValueLocal[0] =(float)(ADC_ConvertedValue&0xFFF)*3.3/4096; 
	  ADC_ConvertedValueLocal[1] =(float)((ADC_ConvertedValue>>16)&0xFFF)*3.3/4096;         
		printf("\nADC1转换原始值 = %04d --> 电压值 = %f V ", ADC_ConvertedValue&0xFFFF,ADC_ConvertedValueLocal[0]);    
    printf("\nADC2转换原始值 = %04d --> 电压值 = %f V ", (ADC_ConvertedValue>>16)&0xFFFF,ADC_ConvertedValueLocal[1]);      
    printf("\n已经完成AD转换次数：%d",DMA_Transfer_Complete_Count);
    printf("\r\n");    
